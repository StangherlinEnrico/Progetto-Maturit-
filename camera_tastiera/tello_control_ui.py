...
class TelloUI:
    ...
    def __init__(self,tello,outputpath):
        ...
        # create buttons
        self.btn_snapshot = tki.Button(self.root, text="Schermata!",
                                       command=self.takeSnapshot)
        self.btn_snapshot.pack(side="bottom", fill="both",
                               expand="yes", padx=10, pady=5)

        self.btn_pause = tki.Button(self.root, text="Pausa", relief="raised", command=self.pauseVideo)
        self.btn_pause.pack(side="bottom", fill="both",
                            expand="yes", padx=10, pady=5)

        self.btn_landing = tki.Button(
            self.root, text="Pannello di controllo", relief="raised", command=self.openCmdWindow)
    ...
    def openCmdWindow(self):
        ...
        panel.wm_title("Pannello di controllo")
        ...
        text1 = tki.Label(panel, text=
                          ...
                          'A - Rotate Counter-Clockwise\t\tArrow Left - Move Left\n'
                          'D - Rotate Clockwise\t\t\tArrow Right - Move Right',
                          justify="left")
        text1.pack(side="top")

        self.btn_landing = tki.Button(
            panel, text="Atterra", relief="raised", command=self.telloLanding)
        self.btn_landing.pack(side="bottom", fill="both",
                              expand="yes", padx=10, pady=5)

        self.btn_takeoff = tki.Button(
            panel, text="Decolla", relief="raised", command=self.telloTakeOff)
        ...
        self.btn_landing = tki.Button(
            panel, text="Capovolta", relief="raised", command=self.openFlipWindow)
        self.btn_landing.pack(side="bottom", fill="both",
                              expand="yes", padx=10, pady=5)

        self.distance_bar = Scale(panel, from_=0.02, to=5, tickinterval=0.01, digits=3, label='Distanza(m)',
                                  resolution=0.01)
        self.distance_bar.set(0.2)
        self.distance_bar.pack(side="left")

        self.btn_distance = tki.Button(panel, text="Aggiorna distanza", relief="raised",
                                       command=self.updateDistancebar,
                                       )
        self.btn_distance.pack(side="left", fill="both",
                               expand="yes", padx=10, pady=5)

        self.degree_bar = Scale(panel, from_=1, to=360, tickinterval=10, label='Angolo')
        self.degree_bar.set(30)
        self.degree_bar.pack(side="right")

        self.btn_distance = tki.Button(panel, text="Aggiorna angolo", relief="raised", command=self.updateDegreebar)
    ...
    def openFlipWindow(self):
        ...
        self.btn_flipl = tki.Button(
            panel, text="Capovolta a sinistra", relief="raised", command=self.telloFlip_l)
        self.btn_flipl.pack(side="bottom", fill="both",
                            expand="yes", padx=10, pady=5)

        self.btn_flipr = tki.Button(
            panel, text="Capovolta a destra", relief="raised", command=self.telloFlip_r)
        self.btn_flipr.pack(side="bottom", fill="both",
                            expand="yes", padx=10, pady=5)

        self.btn_flipf = tki.Button(
            panel, text="Capovolta in avanti", relief="raised", command=self.telloFlip_f)
        self.btn_flipf.pack(side="bottom", fill="both",
                            expand="yes", padx=10, pady=5)

        self.btn_flipb = tki.Button(
            panel, text="Capovolta all\'indietro", relief="raised", command=self.telloFlip_b)
